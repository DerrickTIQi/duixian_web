    ...
    <el-scrollbar class="mscroll">
      ...
      >
        ...
        <el-table-column prop="time" label="时间" align="center" width="100px">
          <template #default="{ row }">
            <a :href="getRowLink(row)" class="row-link">
              <span style="font-weight: bold;">{{ row.odds_datetime?.date_cut }}</span>
            </a>
          </template>
        </el-table-column>
        <el-table-column prop="status" label="状态" align="center" width="75px">
          <template #default="{ row }">
            <a :href="getRowLink(row)" class="row-link">
              <span v-if="(route.path.split('/')[2] === 'record') && row.NCN?.TIMER >= 90" style="display: flex; justify-content: center; align-items: center;">
                <img src="@/assets/table/icon-over.png" style="margin-right: 8px"/>
                完
              </span>
              <span v-else-if="row.NCN?.TIMER == 0">未开始</span>
              <div v-else class="status-time" style="color: #ff3b30; font-weight: bold;">
