...
filterEarly().then((res) => {
    naviItem.value = res.date.map(item => formatDate(new Date(item.date))); // 统一格式化为 `MM-DD`
...
const fetchPageData = async () => {
    ...
    try {
        const res = await listEarly(body);
        ...
        while (allData.length < pageSize.value) {
            ...
            // 请求下一页的数据
            const ress = await listEarly(nextPageBody);
...
//         return; // 如果没有更多页或正在加载，直接返回
//     }else{
//         currentPage.value++; // 增加当前页
//         await fetchPageData(); // 加载下一页数据
//     }
